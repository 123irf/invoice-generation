...
export function InvoiceForm({ initial, clients, predefinedItems, defaults, taxSettings }: Props) {
  ...
  const [state, setState] = useState({
    ...
    dueDate: initial?.dueDate ?? defaultDueDate(defaults.dueDateDays),
  ...
  function save(asStatus: 'DRAFT' | 'SENT') {
    ...
    startTransition(async () => {
      try {
        ...
        if (initial) {
          const r = await updateInvoice(initial.id, payload);
          if (r.ok) {
          ...
          // createInvoice redirects on success
        }
      } catch (e: unknown) {
  ...
  return (
    <div className="flex gap-8">
      ...
      <div className="w-72 shrink-0 space-y-4">
        ...
        {/* Invoice Details box */}
        <div className="bg-white rounded-lg border border-slate-200 p-4 space-y-3">
          <h3 className="font-semibold text-sm text-slate-900">Invoice Details</h3>
