...
Build the admin dashboard at `/dashboard` with key stat cards and a recent activity feed.
This is the first thing the admin sees after sign-in.
...
- **Four stat cards** at the top (responsive grid)
  1. Quotes Pending — count of quotes with status SENT
  2. Invoices Unpaid — count of invoices with status SENT or PARTIAL
  3. Overdue — count of invoices where status SENT and dueDate < now
  4. Collected This Month — sum of payment amounts for current month
- **Two columns** below:
  - Recent Quotes (5 most recent) with status badges
  - Recent Invoices (5 most recent) with status badges + payment progress
- **Quick Actions row** at the top right: New Quote, New Invoice, Add Client

## Steps

### 1. Replace the placeholder Dashboard page
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex gap-2">
          <Button asChild variant="outline" size="sm">
            <Link href="/clients"><Plus className="h-4 w-4 mr-1" /> Client</Link>
          </Button>
          <Button asChild variant="outline" size="sm">
            <Link href="/quotes/new"><Plus className="h-4 w-4 mr-1" /> Quote</Link>
          </Button>
          <Button asChild size="sm">
            <Link href="/invoices/new"><Plus className="h-4 w-4 mr-1" /> Invoice</Link>
          </Button>
        </div>
      </div>
...
### 2. Optional: Sparkline / chart for "Collected This Month"

If you want a tiny visual flourish, plot a 30-day collection sparkline using `recharts`
(already in dependencies via shadcn). Skip for MVP if pressed for time.

## Verification Checklist

- [ ] Sign in → land on `/dashboard`
- [ ] Four stat cards render with correct counts and amounts
- [ ] Clicking a stat card navigates to the filtered list (e.g. Quotes Pending → `/quotes?status=SENT`)
- [ ] Recent Quotes shows up to 5 most recently created quotes with status badges
- [ ] Recent Invoices shows progress (paid of total) and OVERDUE badge for past-due unpaid invoices
- [ ] "Collected This Month" only sums payments from the current calendar month
- [ ] Quick action buttons at top-right navigate to the create flows
...
```bash
git add -A
git commit -m "step-12: admin dashboard with stat cards and recent activity feed"
```

## Next

Proceed to `13-SEED-DEPLOY.md`.
